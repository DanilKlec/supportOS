...
export default async function handler(req,res) {
 ...
 try {
  ...
  const actor=await requireUser(req,{permission:`${permission}.${req.method==='GET'?'read':'write'}`});const env=config();
  if(req.method==='GET')return send(200,(await db(env,`supportos_shared_content?id=eq.${kind}&select=*`))[0]??null);
  ...
  const response=await fetch(`${env.SUPABASE_URL.replace(/\/$/,'')}/rest/v1/rpc/${kind==='binds'?'supportos_import_common_binds':'supportos_publish_content'}`,{
   ...
   body:JSON.stringify(kind==='binds'?{actor:actor.id,payload:body.data}:{actor:actor.id,dataset:kind,expected:body.expected,payload:body.data}),signal:AbortSignal.timeout(20000)
  });
