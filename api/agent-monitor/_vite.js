...
export function agentMonitorPlugin() {
 return {name:'supportos-agent-monitor',configureServer(server) {
  ...
  server.middlewares.use(async(req,res,next)=>{
   ...
   if(!['/api/agent-monitor','/api/accounts','/api/ai/knowledge','/api/binds'].includes(path)) return next();
   try {
    let raw='';
    ...
    req.body=raw?JSON.parse(raw):{};await (path==='/api/binds'?binds:path==='/api/accounts'?accounts:path==='/api/ai/knowledge'?knowledge:handler)(req,res);
