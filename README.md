...
Мониторинг приёма чатов LiveChat: `/agent-monitor` → [настройка сервера, базы и сборщика](docs/agent-monitor.md). Статусы и журнал по сменам GMT+3 требуют серверного подключения; инструкция содержит все переменные и SQL-миграцию.
...
Приложение использует Supabase Auth для входа. Данные базы знаний сохраняются в IndexedDB браузера; облачная синхронизация включается отдельно.
...
- Supabase Auth, защищённые маршруты, роли руководителей и отдельная опция облачной синхронизации.
