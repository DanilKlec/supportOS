...
it("handles inclusive expiry, due review and unverified legacy bonuses", () => {
	...
	expect(
		bonusStatus(
			{ validUntil: "2026-09-10", checkedAt: "2026-09-10" },
			"2026-09-11",
		).label,
	).toBe("Срок истёк");
	expect(
		bonusStatus(
			{ validUntil: "2026-09-11", checkedAt: "2026-09-10" },
			"2026-09-11",
		).label,
	).toBe("Проверен");
	expect(
		bonusStatus(
			{ reviewDue: "2026-09-11", checkedAt: "2026-09-10" },
			"2026-09-11",
		).label,
	).toBe("Пора проверить");
});
it("validates metadata and continues to accept existing catalogs", () => {
	const check = (extra: Record<string, unknown>) =>
		validContent("bonuses", [
			{
				id: "p",
				slug: "p",
				name: "P",
				bonuses: [{ id: "b", name: "B", content: "C", order: 0, ...extra }],
			},
		]);
	...
	expect(check({ validUntil: "2026-02-30" })).toBe(false);
	expect(check({ responsible: "x".repeat(121) })).toBe(false);
	expect(check({ checkedAt: "bad" })).toBe(false);
});
