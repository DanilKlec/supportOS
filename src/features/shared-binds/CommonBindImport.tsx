...
export function CommonBindImport() {
	...
	const preview = async (input: unknown) => {
		...
		const prepared = source.map((r: any, i: number) => {
			if (
				!r ||
				...
				throw new Error(
					`Бинд ${i + 1}: заполните язык, название и текст переводов`,
				);
			if (
				new Set(r.translations.map((t: any) => t.language)).size !==
				r.translations.length
			)
			...
			const slug =
				typeof r.slug === "string" && r.slug.trim()
					? r.slug.trim()
					: r.translations[0].title.trim().toLowerCase();
			...
			return {
				...
				translations: r.translations.map((t: BindTranslation) => ({
					language: t.language,
					title: t.title.trim(),
