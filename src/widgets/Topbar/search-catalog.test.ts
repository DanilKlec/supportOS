...
it("indexes separate mail addresses and multilingual bonus content", () => {
				...
				bonuses: [
					{
						...
						order: 0,
						translations: [
							{ language: "en", content: "Deposit reward", updatedAt: "" },
						],
					},
				],
			},
	...
	expect(
		rows.find((r) => r.resultKind === "bonus")?.translations[0].content,
	).toBe("Deposit reward");
	expect(
		searchBinds(rows, "verify@example.com", {
			language: "ru",
			categories: [],
			folders: [],
		})[0].id,
