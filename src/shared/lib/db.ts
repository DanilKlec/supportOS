...
function getAll<T>(storeName: string): Promise<T[]> {
	return tx<T[]>(storeName, "readonly", (store) => store.getAll()) as Promise<
		T[]
	>;
}
...
export async function getBind(id: string): Promise<Bind | undefined> {
	return tx<Bind | undefined>("binds", "readonly", (store) =>
		store.get(id),
	) as Promise<Bind | undefined>;
}
...
export async function getPreferences(): Promise<Preferences> {
	const result = await tx<Preferences | undefined>(
		"preferences",
		"readonly",
		(store) => store.get("prefs"),
	);
...
export async function setSelectedLanguage(
...
export async function trackRecent(bindId: string): Promise<void> {
	...
	prefs.recentIds = [
		bindId,
		...prefs.recentIds.filter((id) => id !== bindId),
	].slice(0, MAX_RECENT);
	await put("preferences", prefs);
}
