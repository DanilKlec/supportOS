		...
		return (
			<div className="flex h-full flex-col items-center justify-center rounded-3xl border border-border bg-surface p-8 text-center">
				...
				<h2 className="text-2xl font-semibold text-foreground mb-2">
					Select a bind to open
				</h2>
				<p className="text-sm text-muted max-w-md">
					Use the Knowledge tree or workspace modes to choose a bind. The editor
					will stay open while you browse.
				</p>
			</div>
		);
	}
		...
		<div className="flex h-full flex-col rounded-3xl border border-border bg-surface p-6">
			<div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
				...
				<div className="ui-actions items-center flex flex-wrap  gap-2">
					<button
						type="button"
						className="ui-button ui-button--primary inline-flex items-center gap-2 bg-accent font-medium text-accent-foreground hover:bg-accent/90 transition-colors"
					>
						<Terminal size={16} />
						Copy current
					</button>
					...
					<button
						type="button"
						className="ui-button ui-button--ghost inline-flex items-center gap-2 bg-surface-elevated font-medium text-foreground hover:bg-surface transition-colors"
					>
						<Edit2 size={16} />
						Edit
					</button>
					<button
						type="button"
						className="ui-button ui-button--ghost inline-flex items-center gap-2 bg-surface-elevated font-medium text-muted hover:bg-surface transition-colors"
					>
						<Clock size={16} />
						History
					</button>
					...
					<div className="grid grid-cols-2 gap-2">
						{SUPPORTED_LANGUAGES.map((language) => (
							<button
								...
								onClick={() =>
									handleCopy(
										bind.translations[language.code]?.command ||
											bind.translations[language.code]?.content ||
											"",
									)
								}
							...
							>
								<div className="font-medium">
									Copy {language.code.toUpperCase()}
								</div>
								<div className="text-xs text-muted">
									{bind.translations[language.code]?.title || "No translation"}
								</div>
							</button>
						))}
					</div>
			...
			<div className="flex-1 overflow-hidden rounded-3xl border border-border bg-background p-6"></div>
		</div>
	);
}
